...
export function useCarousel(
  ...
  const { ref, onMouseDown } = useSwipeable({
    onSwiping(e) {
      ...
      dispatch({
        type: 'drag',
        offset: sign * Math.min(Math.abs(e.deltaX), limit * (container as any).clientWidth),
      });
    },
    onSwipedLeft(e) {
      swiped(e.deltaX, dispatch, length, 1, container as any);
    },
    onSwipedRight(e) {
      swiped(e.deltaX, dispatch, length, -1, container as any);
    },
  ...
  const handlers = {
    ...
    ref(container: HTMLElement) {
      setContainer((container as any).firstElementChild);
  ...
  return [state.active, n => dispatch({ type: 'jump', desired: n }), handlers as any, style];
}